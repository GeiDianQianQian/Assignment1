...
def pivot_months_pandas(data):
    ...
    """
    table = data.copy()
    table['date'] = table['date'].apply(date_to_month)
    #print(table['date'])
    monthly_num = table.groupby(['name','date'])['precipitation'].aggregate('sum').reset_index()
    #print(monthly_num)
    counts_num = table.groupby(['name','date'])['precipitation'].aggregate('count').reset_index()

    monthly = monthly_num.pivot(index='name', columns='date', values='precipitation')
    counts = counts_num.pivot(index='name', columns='date', values='precipitation')
